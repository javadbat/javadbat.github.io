    ...
    <div className={styles.tabPanel}>
      {element.children.length === 0 ? <div className={styles.tabEmptyDrop} data-drop-active={dragOverIndex === 0} onDragOver={event => markDropTarget(event, 0)} onDrop={event => acceptDrop(event, 0)}>Drop form elements into this repeatable group</div> :
        <ol className={styles.tabChildList}>{element.children.map((child, index) => <li key={child.id}>
          <div className={styles.insertionTarget} data-active={dragOverIndex === index} onDragOver={event => markDropTarget(event, index)} onDrop={event => acceptDrop(event, index)}><span><CatalogIcon iconId="drop" />{messages.dropHere}</span></div>
          ...
          {index === element.children.length - 1 ? <div className={styles.insertionTarget} data-active={dragOverIndex === element.children.length} onDragOver={event => markDropTarget(event, element.children.length)} onDrop={event => acceptDrop(event, element.children.length)}><span><CatalogIcon iconId="drop" />{messages.dropHere}</span></div> : null}
        </li>)}</ol>}
    </div>
  </div>;
});
