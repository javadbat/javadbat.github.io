...
export class SkillsBackground extends HTMLElement {
  ...
  connectedCallback() {
    this.#resizeObserver = new ResizeObserver(() => this.#setPositions());
  ...
  #setPositions() {
    ...
    if (!this.#renderer || !this.#camera || !this.#scene || width <= 0 || height <= 0) {
      return;
    }
    ...
    if (!this.#shapes.core || !this.#shapes.earth || !this.#shapes.jupiter) {
      return;
    }
    ...
    this.#setModelSize(this.#shapes.earth, maxSize);
    //jupiter scale
    const jupiterMaxSize = (this.sphereRadius * 2 * 2) / 6;
    this.#setModelSize(this.#shapes.jupiter, jupiterMaxSize);
    //pos
  ...
  #setModelSize(model: THREE.Group<THREE.Object3DEventMap>, targetSize: number) {
    model.scale.setScalar(1);
    model.updateMatrixWorld(true);
    model.scale.setScalar(this.#calcModelScale(model, targetSize));
  }
